...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  function seatNumberValidation(agent){
  ...
  function originHandler(agent){
    agent.add(`what's the origin of the flight?`);
    agent.add(`     `);
    agent.add(`Quick Suggestions`);
    agent.add(new Suggestion(`delhi`));
    agent.add(new Suggestion(`chennai`));
    agent.add(new Suggestion(`varanasi`));
    agent.add(new Suggestion(`kolkata`));
    agent.add(new Suggestion(`gujraat`));
    agent.add(new Suggestion(`mumbai`));
    agent.add(new Suggestion(`pune`));
    agent.add(new Suggestion(`banglaore`));
  }
  
  function destinationAfterOriginHandler(agent){
    console.log(JSON.stringify(agent.paramters));
  ...
  function moveToTravelDate(agent){
  	 let  {origin,destination} = agent.parameters;
     if(origin !== destination)
     agent.setFollowupEvent(`TRAVEL_EVENT`); 
  }
  
  function goToTravelDateHandler(agent){
    console.log('goToTravelDateHandler',JSON.stringify(agent));
    agent.setFollowupEvent(`TRAVEL_EVENT`); 
  }
  
  function destinationRequiredOnlyHandler(agent){
  	agent.add(`What's the destination of flight?`);
    agent.add(`     `);
    agent.add(`Quick Suggestions`);
    agent.add(new Suggestion(`delhi`));
  ...
  intentMap.set('FlightBooking.TravelDate.ClassType.Seat.Yes.SeatNumber',seatNumberValidation);
  intentMap.set('Payment',paymentHandler);
  intentMap.set('Payment.ConversationEnd',conversationEndHandler);
  intentMap.set('FlightBooking.Origin',originHandler);
  intentMap.set('FlightBooking.Origin.Destination',destinationAfterOriginHandler);
  intentMap.set('FlightBooking.Origin.Destination.Dates',moveToTravelDate);
  intentMap.set('FlightBooking.Destination',destinationRequiredOnlyHandler);
  intentMap.set('FlightBooking.Destination.Dates', goToTravelDateHandler);
